...
export class FormRegionaladdressComponent extends AbstractReactive
  ...
  onSelectState(id: any) {
    ...
    // Filter and get data
    this.subscribe = this.addressService
      .getMunicipalityByState(this.state.value)
      .subscribe((response) => {
  ...
  onConfirm() {
    ...
    switch (this.MODE) {
      case this.CRUD.CREATE:
        this.addressService.setMunicipality(data).subscribe(
          (response) => {
      ...
      case this.CRUD.EDIT:
        // Parse id and rename
        this.addressService
          .updateMunicipality(this.municipality.value, {
            name: this.inputCRUD.value,
            state: this.state.value,
          })
          .subscribe(
      ...
      case this.CRUD.DELETE:
        this.addressService
          .deleteMunicipality(this.municipality.value)
          .subscribe((response) => {
            // Remove the array's item
            this.municipalities = this.municipalities.filter(
              (item) => item.id !== this.municipality.value
            );
